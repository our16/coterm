...
$script:CoTermShorthandNames = @('list','status','run','read','info','create','env','stop','deactivate','off','interrupt','close','record','replay','snapshot','restore')

# Wrap the prompt so an active environment shows a "(coterm) " prefix.
...
# Shorthand commands (list, status, run, ...) become available while the
# environment is active, so you don't need the "coterm " prefix.
function script:CoTerm-DefineShorthands {
  foreach ($n in $script:CoTermShorthandNames) {
    & ([scriptblock]::Create("function global:$n { coterm $n @args }"))
  }
}
function script:CoTerm-RemoveShorthands {
  foreach ($n in $script:CoTermShorthandNames) {
...
function global:coterm {
  ...
  if ($null -eq $cmd -or $cmd -eq 'activate' -or $cmd -eq 'on') {
    ...
    script:CoTerm-DefineShorthands
  } elseif ($cmd -eq 'stop' -or $cmd -eq 'deactivate' -or $cmd -eq 'off') {
    $script:CoTermPromptModifier = ''
    script:CoTerm-RemoveShorthands
  }
  return $code
}
`;
