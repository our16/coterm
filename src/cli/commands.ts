...
# Prompt: show "(coterm) " only in THIS window (its per-window file
# ~/.config/coterm/active-$PID exists after this window ran activate).
...
if (-not $script:CoTermPromptWrapped) {
  ...
  function global:prompt {
    $prefix = ''
    if (Test-Path $CoTermMarker) { $prefix = '(coterm) ' }
